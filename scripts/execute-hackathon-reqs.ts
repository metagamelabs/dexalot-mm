...
import { fetchTradingPairs, fetchDeploymentAbi, addBuyLimitOrder, addSellLimitOrder } from "../src/dexalot-tasks";
import _ from "lodash"
import C from "../src/constants";
import { Contract, Wallet, BigNumber } from "ethers";

// init args
const INIT_ARGS = {predefinedSpread: 0.01, midPrice: 0.1, clearOrderBookOnStart: false,
...
async function initWeb3Stuff() {
    WALLET = await new ethers.Wallet("8e9cdb3e5c49c5382c888772e0651cb62d89837fcddb7beb5875a2cf6e412d45",  
...
async function main() {
  ...
  // Add funds if needed
  const team6Balance = await PortfolioContract.getBalance(WALLET.address, B32(TEAM6_AVAX_PAIR.quote));
  console.log("Team 6 Available Balance: ", team6Balance.available);
  const minTeam6AvailableAmount = ethers.utils.parseUnits("20", TEAM6_AVAX_PAIR.base_evmdecimals);
  if (team6Balance.available.lt(minTeam6AvailableAmount)) {
    console.log("DEPOSTING: ", minTeam6AvailableAmount.sub(team6Balance.available));
    const depositTeam6TokensTx = await PortfolioContract.depositToken(WALLET.address, B32(TEAM6_AVAX_PAIR.base), minTeam6AvailableAmount.sub(team6Balance.available));
    await depositTeam6TokensTx.wait();
    console.log("Done depositing Tokens")
  }
  const avaxBalance = await PortfolioContract.getBalance(WALLET.address, B32(TEAM6_AVAX_PAIR.base));
  ...
  if (avaxBalance.available.lt(minAvaxAvailableAmmount)) {
    const depositAvaxTx = await WALLET.sendTransaction({to: PortfolioContract.address, value: minAvaxAvailableAmmount.sub(avaxBalance.available)})
  ...
  await addBuyLimitOrder(TEAM6_AVAX_PAIR, INIT_ARGS.midPrice, minTradeAmount, TradePairsContract, WALLET);
}
